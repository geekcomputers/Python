...
def create_styled_frame(
    parent: QtWidgets.QWidget, min_size: tuple[int, int] | None = None, style: str = ""
) -> QtWidgets.QFrame:
...
def create_styled_label(
    ...
    font_size: int = 12,
    bold: bool = False,
    style: str = "color: #2c3e50; padding: 10px;",
) -> QtWidgets.QLabel:
...
def create_styled_button(
    parent: QtWidgets.QWidget, text: str, min_size: tuple[int, int] | None = None
...
def create_input_field(
    parent: QtWidgets.QWidget,
    label_text: str,
    min_label_size: tuple[int, int] = (120, 0),
...
def search_result(
    parent: QtWidgets.QWidget, title: str, label_text: str
) -> tuple[QtWidgets.QWidget, tuple[QtWidgets.QLineEdit, QtWidgets.QPushButton]]:
    ...
    user_frame, user_account_number = create_input_field(
        form_frame, label_text, min_label_size=(180, 0)
    )
    ...
    content_layout.addWidget(
        form_frame, alignment=QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter
    )
...
def create_page_with_header(
    parent: QtWidgets.QWidget, title_text: str
    ...
    header_layout.addWidget(
        title_label, alignment=QtCore.Qt.AlignHCenter | QtCore.Qt.AlignTop
    )
...
def get_employee_name(
    parent: QtWidgets.QWidget, name_field_text: str = "Enter Employee Name"
) -> QtWidgets.QWidget:
...
def create_login_page(
    parent: QtWidgets.QWidget,
    title: str,
    name_field_text: str = "Name :",
    password_field_text: str = "Password :",
    submit_text: str = "Submit",
) -> tuple[
    QtWidgets.QWidget, QtWidgets.QLineEdit, QtWidgets.QLineEdit, QtWidgets.QPushButton
]:
...
def create_home_page(
    parent: QtWidgets.QWidget,
    on_admin_clicked: Callable[[], None],
    on_employee_clicked: Callable[[], None],
    ...
    button_layout.addWidget(
        button_container, alignment=QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter
    )
...
def show_employee_list_page(parent: QtWidgets.QWidget, title: str) -> QtWidgets.QWidget:
    ...
    try:
        ...
        error_label = QtWidgets.QLabel(
            f"Error loading employees: {str(e)}", table_frame
        )
...
def create_account_page(
    parent: QtWidgets.QWidget, title: str, update_btn: bool = False
) -> tuple[
    QtWidgets.QWidget,
    tuple[
        QtWidgets.QLineEdit,
...
def create_show_details_page2(
    ...
    for label in labels:
        label_frame, input_field = create_input_field(
            form_frame, label, min_label_size=(180, 30)
        )
...
def update_user(
    parent: QtWidgets.QWidget,
    title: str,
    input_fields_label: str,
    input_field: bool = True,
) -> tuple[
    QtWidgets.QWidget,
    tuple[
        QtWidgets.QLineEdit,
    ...
    | tuple[QtWidgets.QLineEdit, QtWidgets.QLineEdit, QtWidgets.QPushButton],
]:
    ...
    user_frame, user_account_name = create_input_field(
        form_frame, "User Name: ", min_label_size=(180, 0)
    )
    balance_frame, user_balance = create_input_field(
        form_frame, "Balance: ", min_label_size=(180, 0)
    )
    ...
    user_balance.setStyleSheet(
        "background-color: #8a8a8a; border: 1px solid #ccc; border-radius: 4px; padding: 8px;"
    )

    amount_field: QtWidgets.QLineEdit | None = None
    if input_field:
        amount_frame, amount_field = create_input_field_V(
...
def setup_main_window(
...
) -> tuple[QtWidgets.QStackedWidget, dict]:
    ...
    def admin_login_menu_page(name: str, password: str) -> None:
        try:
            ...
            QtWidgets.QMessageBox.critical(
                stacked_widget, "Error", f"Login error: {str(e)}"
            )

    def add_employee_form_submit(
        name: str, password: str, salary: str, position: str
    ) -> None:
    ...
    def handle_update_employee_data(
        name: str, password: str, salary: str, position: str, name_to_update: str
    ) -> None:
        try:
            ...
            show_popup_message(
                stacked_widget, f"Update error: {str(e)}", UPDATE_EMPLOYEE_PAGE2
            )
    ...
    add_employee_page, emp_name, emp_password, emp_salary, emp_position, emp_submit = (
        create_add_employee_page(stacked_widget, "Add Employee")
    )
    emp_submit.clicked.connect(
        lambda: add_employee_form_submit(
            emp_name.text().strip(),
            emp_password.text().strip(),
            emp_salary.text().strip(),
            emp_position.text().strip(),
        )
    )
    ...
    stacked_widget.currentChanged.connect(
        lambda index: populate_employee_data()
        if index == UPDATE_EMPLOYEE_PAGE2
        else None
    )
    u_employee_update.clicked.connect(
        lambda: handle_update_employee_data(
    ...
    employee_page, employee_name, employee_password, employee_submit = (
        create_login_page(stacked_widget, "Employee Login")
    )
    ...
    e_check_balance.clicked.connect(
    ...
    employee_create_account_page, create_account_fields = create_account_page(
    ...
    def handle_create_account() -> None:
        try:
            ...
            show_popup_message(
                stacked_widget, "Account created successfully", EMPLOYEE_MENU_PAGE
            )
        except Exception as e:
            show_popup_message(
                stacked_widget,
    ...
    show_details_page1, show_details_widgets1 = create_show_details_page1(
        stacked_widget, "Show Details"
    )
    show_details_input1, show_details_btn1 = show_details_widgets1

    show_details_page2, show_details_widgets2 = create_show_details_page2(
        stacked_widget, "Account Details"
    )
    (
        acc_no_field,
        name_field,
    ...
    def handle_show_details_submit() -> None:
        try:
            ...
            show_popup_message(
                ...
                EMPLOYEE_SHOW_DETAILS_PAGE1,
            )
        except Exception as e:
            show_popup_message(
                stacked_widget, f"Error: {str(e)}", EMPLOYEE_SHOW_DETAILS_PAGE1
            )

    show_details_btn1.clicked.connect(handle_show_details_submit)

    def setup_balance_operation(
        title_search: str,
        placeholder: str,
        title_form: str,
        action_text: str,
        ...
        form_page, form_widgets = update_user(
            stacked_widget, title_form, action_text, need_input
        )
        ...
        def on_search() -> None:
            try:
                ...
                show_popup_message(
                    stacked_widget, "Please enter a valid account number", search_idx
                )
            except Exception as e:
                show_popup_message(
                    stacked_widget, f"Search error: {str(e)}", search_idx
                )
        ...
        def on_action() -> None:
            try:
                ...
                show_popup_message(
                    stacked_widget, "Please enter a valid amount", page_idx
                )
            except Exception as e:
                show_popup_message(
                    stacked_widget, f"Operation error: {str(e)}", page_idx
                )
    ...
    add_balance_search_page, add_balance_page = setup_balance_operation(
        "Add Balance",
        ...
        "update_balance",
        EMPLOYEE_ADD_BALANCE_SEARCH,
        EMPLOYEE_ADD_BALANCE_PAGE,
    )

    withdraw_search_page, withdraw_page = setup_balance_operation(
        "Withdraw Money",
        "Enter Account Number: ",
        "Withdraw Money",
        "Withdraw Amount: ",
        ...
        EMPLOYEE_WITHDRAW_MONEY_PAGE,
    )

    check_balance_search_page, check_balance_page = setup_balance_operation(
        "Check Balance",
        "Enter Account Number: ",
        "Check Balance",
        "",
        ...
        EMPLOYEE_CHECK_BALANCE_PAGE,
        False,
    )

    update_account_search_page, update_account_search_widgets = search_result(
        stacked_widget, "Update Account", "Enter Account Number: "
    )
    update_account_input, update_account_btn = update_account_search_widgets

    update_account_page, update_account_fields = create_account_page(
        stacked_widget, "Update Account", update_btn=True
    )
    (
        u_name_edit,
        u_age_edit,
    ...
    def handle_update_search() -> None:
        try:
            ...
            if account_data:
                ...
                show_popup_message(
                    stacked_widget, "Account not found", EMPLOYEE_UPDATE_ACCOUNT_SEARCH
                )
        except ValueError:
            show_popup_message(
                stacked_widget,
                "Please enter a valid account number",
                EMPLOYEE_UPDATE_ACCOUNT_SEARCH,
            )
        except Exception as e:
            show_popup_message(
                stacked_widget, f"Error: {str(e)}", EMPLOYEE_UPDATE_ACCOUNT_SEARCH
            )
    ...
    def handle_account_update() -> None:
        try:
            ...
            show_popup_message(
                stacked_widget, "Account updated successfully", EMPLOYEE_MENU_PAGE
            )
        except ValueError:
            show_popup_message(
                stacked_widget, "Invalid account number", EMPLOYEE_UPDATE_ACCOUNT_PAGE
            )
        except Exception as e:
            show_popup_message(
                stacked_widget, f"Update error: {str(e)}", EMPLOYEE_UPDATE_ACCOUNT_PAGE
            )
