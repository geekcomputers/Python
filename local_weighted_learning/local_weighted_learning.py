...
def weighted_matrix(point: np.mat, training_data_x: np.mat, bandwidth: float) -> np.mat:
    ...
    It takes training_point , query_point, and tau Here Tau is not a fixed
    value it can be varied depends on output. tau --> bandwidth xmat
    -->Training data point --> the x where we want to make predictions
    """
...
def local_weight(
    ...
    """Calculate the local weights using the weight_matrix function on training
    data.
...
def load_data(dataset_name: str, cola_name: str, colb_name: str) -> np.mat:
    """Function used for loading data from the seaborn splitting into x and y
    points."""
