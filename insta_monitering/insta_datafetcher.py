    ...
    try:
        ...
        for script in scripts:
            if "window._sharedData =" in script.text:
                ...
                json_text = (
                    json_text.replace("window._sharedData =", "")
                    .replace(";", "")
                    .strip()
                )
                ...
                if "entry_data" in data and "PostPage" in data["entry_data"]:
                    post = data["entry_data"]["PostPage"][0]["graphql"][
                        "shortcode_media"
                    ]
                    return {
                        "id": post.get("id"),
...
class MonitoringClass:
    ...
    def _initialize(self):
        ...
        try:
            self.client = pymongo.MongoClient(
                host=self.db_host, port=self.db_port, serverSelectionTimeoutMS=5000
            )
            ...
            if self.monitor_type == "hashtags":
                ...
                raise ValueError(
                    f"Invalid monitor type: {self.monitor_type}. Must be 'hashtags' or 'profile'."
                )
    ...
    def _parse_instagram_response(self, response_data):
        ...
        try:
            ...
            if self.monitor_type == "hashtags":
                if (
                    "graphql" not in response_data
                    or "hashtag" not in response_data["graphql"]
                ):
                ...
                if (
                    "graphql" not in response_data
                    or "user" not in response_data["graphql"]
                ):
...
class InstaProcessManager:
        ...
        try:
            ...
            return (
                collection.count_documents(
                    {"user": user, "tags": tags, "productId": product_id}
                )
                > 0
            )
        except Exception as e:
    ...
    def start_monitoring(
        self, user, tags, monitor_type="hashtags", product_id="insta_monitor"
    ):
        """
...
class DataFetcher:
    ...
    def __init__(
        self, user, tags, product_id="insta_monitor", db_host=None, db_port=None
    ):
        """
    ...
    def get_posts_after_timestamp(self, timestamp, limit=20):
        ...
        try:
            cursor = (
                self.collection.find({"timestamp": {"$gt": timestamp}})
    ...
    def get_posts_before_timestamp(self, timestamp, limit=20):
        ...
        try:
            cursor = (
                self.collection.find({"timestamp": {"$lt": timestamp}})
