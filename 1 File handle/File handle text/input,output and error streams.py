...
def get_valid_file_path() -> str:
    ...
    while attempt < max_attempts:
        ...
        sys.stdout.write(
            f"Enter the name of the file (attempt {attempt}/{max_attempts}, or 'q' to quit): "
        )
        sys.stdout.flush()
        ...
        if not is_valid_path(file_path):
            sys.stderr.write(
                "Error: Invalid path syntax. Please enter a valid file path.\n"
            )
            continue
...
def handle_exception(e: Exception, file_path: str) -> None:
    ...
    if error_type == FileNotFoundError:
        ...
        sys.stderr.write(
            "Suggestion: Ensure you have the necessary permissions to access the file.\n"
        )
