...
def worker(mink: int, maxk: int, conn) -> None:
    ...
    try:
        ...
        conn.send(result)  # Send float directly
        conn.close()
...
def pi(n: int) -> float:
    ...
    parent_conns: list = []
    unit: int = n // 10  # Ensure integer division
    ...
    for conn in parent_conns:
        ...
        if isinstance(result, str) and result.startswith("Error:"):
            print(result)
            sums.append(0.0)  # Add zero if error occurred
        else:
            sums.append(float(result))  # Ensure float conversion
        conn.close()
