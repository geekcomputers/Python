...
def parse_arguments() -> argparse.Namespace:
    ...
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        description="Pixel sorting with audio visualization"
    )
    parser.add_argument(
        "-f",
        "--filename",
...
def step_sort_key(
    bgr: np.ndarray, repetitions: int = SORT_REPETITIONS
) -> tuple[int, float, int]:
...
def save_pixel_data(pixels: np.ndarray, output_path: Path) -> None:
    ...
    try:
        df: pd.DataFrame = pd.DataFrame(
            pixels.reshape(-1, 3), columns=["Blue", "Green", "Red"]
        )
...
def pixels_to_audio(
    ...
    for i in range(num_samples):
        ...
        if waveforms[i] < 0.25:  # Sine wave
            ...
            audio_signal[i] = amplitudes[i] * signal.square(
                2 * np.pi * frequencies[i] * t[i]
            )
        elif waveforms[i] < 0.75:  # Triangle wave
            audio_signal[i] = amplitudes[i] * signal.sawtooth(
                2 * np.pi * frequencies[i] * t[i], width=0.5
            )
        else:  # Sawtooth wave
            audio_signal[i] = amplitudes[i] * signal.sawtooth(
                2 * np.pi * frequencies[i] * t[i]
...
def save_audio(
    audio_signal: np.ndarray, output_path: Path, sample_rate: int = AUDIO_SAMPLE_RATE
) -> None:
