...
class NotepadApp:
    ...
    def _setup_add_tab(self) -> None:
        ...
        Button(button_frame, text="Save Note", command=self._save_note).pack(side=tk.RIGHT, padx=5)
        Button(button_frame, text="Clear", command=self._clear_form).pack(side=tk.RIGHT, padx=5)
    ...
    def _setup_display_tab(self) -> None:
        ...
        Button(search_frame, text="Apply Filter", command=self._apply_filter).pack(side=tk.RIGHT, padx=5)
        ...
        Button(button_frame, text="Refresh", command=self._load_all_notes).pack(side=tk.LEFT, padx=5)
        Button(button_frame, text="Delete Selected", command=self._delete_selected_note).pack(side=tk.RIGHT, padx=5)
    ...
    def _setup_search_tab(self) -> None:
        ...
        Button(search_frame, text="Search", command=self._search_notes).pack(side=tk.RIGHT, padx=5)
        ...
        self.prev_button = Button(nav_frame, text="Previous", command=self._show_previous_result, state=tk.DISABLED)
        self.prev_button.pack(side=tk.LEFT, padx=5)
        
        self.next_button = Button(nav_frame, text="Next", command=self._show_next_result, state=tk.DISABLED)
        self.next_button.pack(side=tk.RIGHT, padx=5)
        
        # Result display
        self.result_title_var = tk.StringVar()
        Label(results_frame, textvariable=self.result_title_var, font=("Arial", 12, "bold")).pack(anchor=tk.W, padx=5, pady=5)
        ...
        Label(results_frame, textvariable=self.search_status_var).pack(anchor=tk.W, padx=5, pady=5)
    ...
    def _load_all_notes(self) -> None:
        ...
        try:
            self.cursor.execute("SELECT id, title, created_at FROM notes ORDER BY created_at DESC")
    ...
    def _apply_filter(self) -> None:
        ...
        try:
            if filter_text:
                self.cursor.execute("SELECT id, title, created_at FROM notes WHERE LOWER(title) LIKE ? ORDER BY created_at DESC", 
                                   (f"%{filter_text}%",))
            else:
                self.cursor.execute("SELECT id, title, created_at FROM notes ORDER BY created_at DESC")
    ...
    def _open_note_viewer(self, note_id: int, title: str, content: str) -> None:
        ...
        def update_note() -> None:
            ...
            try:
                ...
                messagebox.showerror("Database Error", f"Failed to update note: {str(e)}")
        
        Button(button_frame, text="Update Note", command=update_note).pack(side=tk.RIGHT, padx=5)
        Button(button_frame, text="Cancel", command=viewer.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def _delete_selected_note(self) -> None:
        ...
        if messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete the note '{title}'?"):
            try:
                ...
                messagebox.showerror("Database Error", f"Failed to delete note: {str(e)}")
    ...
    def _search_notes(self) -> None:
        ...
        try:
            ...
            if not self.current_results:
                ...
                self.search_status_var.set(f"Displaying result {self.current_index + 1} of {len(self.current_results)}")
                self.prev_button.config(state=tk.NORMAL if len(self.current_results) > 1 else tk.DISABLED)
                self.next_button.config(state=tk.NORMAL if len(self.current_results) > 1 else tk.DISABLED)
