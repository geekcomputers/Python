...
def create_widgets(
    window: tk.Tk, game_list: list[str], team_options: list[str]
) -> None:
    ...
    visitor_dropdown.config(
        font=label_font, bg="white", fg=text_color, relief=tk.RAISED, bd=2
    )
    visitor_dropdown["menu"].config(bg="white", fg=text_color)
    ...
    home_dropdown["menu"].config(bg="white", fg=text_color)
