class OneRepMaxCalculator:
    ...
    def get_user_input(self) -> None:
        ...
        while True:
            try:
                ...
                self.reps_performed = int(
                    input("Enter the number of reps you performed: ")
                )
    ...
    def calculate_one_rep_max(self, formula: str = "epley") -> float:
        ...
        if formula not in self.formulas:
            raise ValueError(
                f"Invalid formula. Available options: {', '.join(self.formulas.keys())}"
            )
    ...
    def display_one_rep_max(self, formula: str = "epley") -> None:
        ...
        print(
            f"Your estimated one-rep max (1RM) using {formula.title()} formula is: {one_rep_max:.2f} kg"
        )
