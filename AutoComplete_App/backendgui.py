...
class AutoComplete:
    ...
    def __init__(self, n=2) -> None:
        ...
        self.conn: sqlite3.Connection = sqlite3.connect(
            "autocompleteDB.sqlite3", autocommit=True
        )
        ...
        if not tables_exist:
            ...
            cursor.execute(
                "INSERT INTO NGramPrediction VALUES (?, ?)", ("ngrampredictions", "{}")
            )
    ...
    def train(self, sentence: str) -> str:
        ...
        ngrams_map: dict[tuple[str], dict[str, int]] = json.loads(
            ngrams_map_str,
            object_hook=lambda d: {tuple(k.split()): v for k, v in d.items()},
        )

        cursor.execute(
            "SELECT value FROM NGramPrediction WHERE name='ngrampredictions'"
        )
        predictions_str: str = cursor.fetchone()[0]
        predictions: dict[tuple[str], dict[str, str | int]] = json.loads(
            predictions_str,
            object_hook=lambda d: {tuple(k.split()): v for k, v in d.items()},
        )
        ...
        for i in range(len(ngrams) - 1):
            ...
            if curr_ngram not in predictions:
                ...
                if (
                    ngrams_map[curr_ngram][next_word]
                    > predictions[curr_ngram]["completion_count"]
                ):
                    predictions[curr_ngram]["completion_word"] = next_word
                    predictions[curr_ngram]["completion_count"] = ngrams_map[
                        curr_ngram
                    ][next_word]

        # Save updated data back to database
        updated_ngrams_map: str = json.dumps(
            {" ".join(k): v for k, v in ngrams_map.items()}
        )
        updated_predictions: str = json.dumps(
            {" ".join(k): v for k, v in predictions.items()}
        )

        cursor.execute(
            "UPDATE NGramMap SET value = ? WHERE name='ngramsmap'",
            (updated_ngrams_map,),
        )
        cursor.execute(
            "UPDATE NGramPrediction SET value = ? WHERE name='ngrampredictions'",
            (updated_predictions,),
        )
